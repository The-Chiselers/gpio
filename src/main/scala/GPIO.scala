...
class GPIO(p: BaseParams) extends Module {
  ...
  // Synchronize GPIO input signals, max sync size is set by parameter, min is one
  val gpioInputSyncShiftReg = RegInit(
    VecInit(Seq.fill(p.dataWidth)(VecInit(Seq.fill(p.maxSyncSize)(false.B)))),
  )
  for (i <- 0 until p.dataWidth) {
    for (j <- 0 until p.maxSyncSize - 1) gpioInputSyncShiftReg(i)(j + 1) :=
      gpioInputSyncShiftReg(i)(j)
    gpioInputSyncShiftReg(i)(0) := io.pins.gpioInput(i)
  }

  // the final bit of the shift reg as chosen by the SYCRONIZER register
  val gpioInputSyncronizedVec: Vec[UInt] = Wire(Vec(p.dataWidth, UInt(1.W)))
  for (i <- 0 until p.dataWidth) {
    // default to 1 if synchronizer is 0 to ensure that there is always one layer of syncronization
    val syncBit = Mux(regs.SYNCRONIZER === 0.U, 1.U, regs.SYNCRONIZER)
    gpioInputSyncronizedVec(i) := gpioInputSyncShiftReg(syncBit)(i)
  }

  val gpioInputSync: UInt = gpioInputSyncronizedVec.asUInt
  ...
  def registerDecodeWrite(addr: UInt): Unit = {
    ...
    when(
      addr >= regs.TRIGGER_LVL0_ADDR.U && addr <= regs.TRIGGER_LVL0_ADDR_MAX.U,
    ) {
      printf(
    ...
    when(
      addr >= regs.TRIGGER_LVL1_ADDR.U && addr <= regs.TRIGGER_LVL1_ADDR_MAX.U,
    ) {
      printf(
    ...
    when(
      ...
      regs.TRIGGER_STATUS := regs.TRIGGER_STATUS &
        ~io.apb.PWDATA(regs.TRIGGER_STATUS_SIZE - 1, 0) // Writing a 1 will clear the status, ignore writes of 0
    }
    ...
    when(addr >= regs.SYNCRONIZER_ADDR.U && addr <= regs.SYNCRONIZER_ADDR_MAX.U) {
      printf(
        "Writing SYNCRONIZER Register, data: %x, addr: %x\n",
        io.apb.PWDATA,
        addr,
      )
      regs.SYNCRONIZER := io.apb.PWDATA(regs.SYNCRONIZER_SIZE - 1, 0)
    }
  }
  ...
  def registerDecodeRead(addr: UInt): Unit = {
    ...
    when(
      addr >= regs.TRIGGER_LVL0_ADDR.asUInt &&
        addr <= regs.TRIGGER_LVL0_ADDR_MAX.U,
    ) {
      printf(
    ...
    when(addr >= regs.SYNCRONIZER_ADDR.U && addr <= regs.SYNCRONIZER_ADDR_MAX.U) {
      printf(
        "READING SYNCRONIZER Register, data: %x, addr: %x\n",
        regs.SYNCRONIZER,
        addr,
      )
      io.apb.PRDATA := regs.SYNCRONIZER
    }
  }
